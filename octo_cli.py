...
class SurveillanceSystem:
    ...
    def get_user_settings(self):
        ...
        print("\nВведите таймауты (в секундах) для каждой камеры по отдельности.")
        print("Формат: просто нажмите Enter чтобы оставить дефолт (10s).")
        for cam_idx in self.camera_indices:
            try:
                ...
                t = int(v)
                if t < 0:
                    print("  Нельзя установить отрицательный таймаут, пропускаю.")
                    continue
                self.MOTION_TIMEOUTS[cam_idx] = t
            except Exception:
                print("  Неверный ввод — пропускаю, оставлено текущее значение.")
